...
if __name__ == '__main__':
    ...
    postCodeMap = loadpostcodemap()  # dictionary lookup. postCodeMap.value['SE15'] ==> 'Lewisham'
    ...
    regions = makerSpaceRDD.filter(lambda line: re.split(r',(?=(?:[^"]*"[^"]*")*[^"]*$)', line) != 'Timestamp')\
                           .map(lambda line: postCodeMap[getpostprefix(line)] if getpostprefix(line) in postCodeMap else "Unknown")
